function [ ins_position, ins_velocity, ins_attitude, gx ] = INSUpdate( gryo_measurement, accel_measurement, delta_t, gps_data, start_position, start_velocity, start_attitude )
    ...
    noise_w(8,8) = 0.05;
    noise_w(9,9) = 0.05;
    ins_variance = zeros(9,9,total_length);
    ins_variance(:,:,1) = eye(9,9)*0.005;
    observe_variance = eye(6,6)*0.005;
    
    % data space alloc for RS-RAIM
    fai = eye(9,9);
    rs = zeros( 21, total_length );
    H = zeros( 21, 18, total_length );
	...
	for k = 2 : total_length
        ...
        if mod(k-1,10) == 0
            ...
            observe_vector(:,k) = ([ins_position(k,:),ins_velocity(k,:)]-gps_data(k,:))';
